...
| **4** (wk 7–8) | ~~Industry page 1 (Fitness)~~ (done 23 Sep) · ~~article 1~~ (done 24 Sep) · referral asks · press pitch | Page + article live; two referral asks sent |
...
- **Duplicate Google profile found.** An older profile, "Rexity Tech-Labs" (business ID 12142606431947245644, 73 views in August), still points to rexity.ai. **Not removed.** Removal would leave an unmanaged public listing that anyone could claim, and "closed" was ruled out by the founder. Instead, **merge requested 24 Sep, Google case 1-8214000041053**: merge Rexity Tech-Labs into Rexity Labs UG, keep both profiles in the account until Google confirms, never mark it closed. An earlier case, 26679541397 (12 Aug), only got a template reply. The duplicate has no Maps pin, so the Maps "suggest an edit → duplicate" route doesn't apply. If Google asks for proof, reply with the HRB extract and a utility bill. Directory listings wait until the merge is done.
...
- **Article 1 shipped early (a Sprint 4 item), 24 Sep:** "Was kostet eine Website mit Kursbuchung?" is live at `/website-mit-kursbuchung-kosten`. About 1,100 words, DE with an EN toggle, and it uses only the published `/preise` figures. Body & Care is the example, without booking numbers. It carries Article + FAQPage + Breadcrumb schema, is in the sitemap, and is linked from `/preise` and `/fitnessstudio-website`. Indexing was requested in GSC. Reuse it for the weekly LinkedIn post.
- Directory submissions deferred by the founder. Next: Apple Business Connect, Gelbe Seiten Grundeintrag, 11880, wlw and Startup-Map don't depend on the Google merge. Bing Places waits for Google case 1-8214000041053.
- **GBP merge, 25 Sep:** case 1-8214000041053 got the same template reply as in August. A reply requesting the merge is drafted in Gmail and waits for the founder to send it. It explains the Maps "duplicate" route isn't possible because there is no pin, lists both profile IDs and asks for no closure and no removal.
